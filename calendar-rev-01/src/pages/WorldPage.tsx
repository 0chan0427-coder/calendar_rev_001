...
export default function WorldPage() {
  ...
  return <div className="world-app">
    ...
    <main className="world-main">
      {view==='home' && myProfile && <>
        {selectedHome ? <section className="mini-home-card friend-home-card">
          <div className="mini-home-top">
            <div>
              <h1>{selectedHome.display_name || '씩씩이'}'s MINI HOME</h1>
              <p>💬 {selectedHome.status_message || '놀러오세요!'}</p>
            </div>
            <div className="visit-stats">TODAY <b>{selectedHome.today_visits}</b>　/　TOTAL <b>{selectedHome.total_visits}</b><br/>❤️ 좋아요 <b>{selectedHome.likes}</b></div>
          </div>
          <WorldScene profile={selectedHome} isOwner={false} onToggleScene={()=>{}} />
          <div className="home-actions">
            <button onClick={like}>{liked?'💗 좋아요 취소':'❤️ 좋아요'}</button>
            <button onClick={()=>openGuestbook()}>💌 방명록</button>
            <button onClick={()=>{setSelectedHome(null);setLiked(false);}}>🏠 내 미니홈피</button>
          </div>
        </section> : <>
          <div className="mini-home-card"><div className="mini-home-top"><div><h1>{myProfile.display_name || '씩씩이'}'s MINI HOME</h1>{editingStatus?<div className="status-edit"><input value={status} onChange={e=>setStatus(e.target.value)} maxLength={60}/><button onClick={saveStatus}>저장</button></div>:<p onClick={()=>setEditingStatus(true)}>💬 {myProfile.status_message} <small>(눌러서 수정)</small></p>}</div><div className="visit-stats">TODAY <b>{myProfile.today_visits}</b>　/　TOTAL <b>{myProfile.total_visits}</b><br/>❤️ 좋아요 <b>{myProfile.likes}</b></div></div><WorldScene profile={myProfile} isOwner onToggleScene={toggleScene} roomItems={roomItems} onMoveItem={moveFurniture} onScaleItem={scaleFurniture} onRemoveItem={removeFurniture} onFrontItem={frontFurniture} onOpenInventory={()=>setDecorate(true)} decorate={decorate} onToggleDecorate={()=>setDecorate(v=>!v)}/><div className="home-actions"><button onClick={()=>setDecorate(v=>!v)}>📦 내 아이템</button><button onClick={()=>setView('shop')}>🛍️ 포인트샵</button><button onClick={()=>openGuestbook()}>💌 방명록 보기</button><button onClick={()=>setView('friends')}>👥 친구 만나기</button></div>
      ...
      {view==='friends' && <section className="world-panel"><h2>👥 1촌 / 친구 미니홈피</h2><p className="muted">다른 멤버의 미니홈피를 방문하면 <b>멤버별 하루 1회 +100P</b>를 받을 수 있어요.</p><div className="friend-grid">{otherProfiles.length===0?<div className="muted">아직 다른 공유캘린더 멤버가 없어요.</div>:otherProfiles.map(p=><button className="friend-card" key={p.user_id} onClick={()=>{setSelectedHome(p);setView('home');}}><span className="friend-avatar">🧑🏻‍🎨</span><b>{p.display_name||'씩씩이'}</b><small>{p.status_message||'놀러오세요!'}</small><em>🏠 미니홈피 방문</em></button>)}</div></section>}
